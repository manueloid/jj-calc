...
filename(np::Int64,nlambda::Int64 = 5) = data_dir(nlambda)*"robustness_np$np.dat" # Saving filename for given number of lambda and given number of particles
...
"""
function robustness_all(np::Int64, tspan::Vector{Float64}, Λf::Float64=50.0; kwargs) For a given number of particles `np` and an initial value `Λf` give the robustness of all the types of protocol for each time in `tspan`.
...
function time_check(np::Int64, time_input::Vector{Float64}, nlambda::Int64 =5)
    ...
    isempty(reduced_time) ? println("nothing to do") : return robustness_all(np, reduced_time;nlambda = nlambda)
end

"""
robustness(np::Int64, time_input::Vector{Float64}, nlambda::Int64 =5) It first checks if there is any data file for the given number of particles and the given number of lambdas. 
    If that is the case, it will check how many times we need to calculate the robustness by filtering the values in `time_input` and the `tf` column in the corresponding DataFrame
    It returns an a DataFrame object that can then be saved in an other file
"""
robustness(np::Int64, time_input::Vector{Float64}, nlambda::Int64 = 5) = isfile(filename(np, nlambda)) ? time_check(np, time_input, nlambda) : robustness_all(np, time_input; nlambda = nlambda)

function robustness_write(np::Int64, df::DataFrame, nlambda::Int64=5) 
